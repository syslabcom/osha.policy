...
membership_tool = getToolByName(app.osha.portal, 'portal_membership')
    ...
    def walk(self):
        ...
        for item in self._walk(
            self.portal.unrestrictedTraverse(
                "/osha/portal/images/photo-competition-2011")):
            if item is not None:
                yield item
    ...
    def _walk(self, node):
        for id, ob in node.ZopeFind(node, search_sub=0):
            if ob.meta_type == "ATImage":
                print "walking to %s" %ob.absolute_url(1)
                yield ob
            else:
                self._walk(ob)

def main():
    portal = makerequest(app.osha.portal)
    ...
    walker = ZopeFindImageWalker(portal, migrator=ATImageToBlobImageMigrator)
    walker.go()
