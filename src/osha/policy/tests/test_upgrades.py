...
class TestRearrangeBlog(unittest.TestCase):
    ...
    def _create_test_data(self):
        ...
        self.portal['en']['about']['director_corner'].invokeFactory(
            ...
            description="The aim of this blog is to bring you news about developments in occupational safety and health across the EU and beyond, and also about EU-OSHA initiatives and activities to fulfill our mission. We are one of the smallest EU agencies and cannot promise to reply to every comment, but we will read them and bear them in mind to shape our future work.",
        ...
        self.portal['en']['about']['director_corner']['blog'].invokeFactory(
            'Topic', 'front-page', title="OSH Blog"
        )
        self.portal['en']['about']['director_corner']['blog'].invokeFactory(
    ...
    def test__convert_blog_front_page_converted(self):
        """Test converting front page from Collection to Document"""
        from osha.policy.upgrades import _convert_blog_front_page
        new_front_page = _convert_blog_front_page(
            obj=self.director_corner_en['front-page'],
            description='Description front page',
            wf_state='private'
        )
        self.assertEquals(new_front_page.portal_type, 'Document')
        self.assertEquals(
            new_front_page.Description(), 'Description front page')
        self.assertEquals(
            sorted(new_front_page.aq_parent.keys()),
            ['blog', 'front-page']
        )
        self.assertEquals(new_front_page.getLayout(), 'blog-view')

    def test__convert_blog_front_page_state_not_published(self):
    ...
    def test__convert_blog_front_page_state_published(self):
        """Test if the right workflow state is set for the front page if
    ...
    def test__convert_blog_front_page_text_no_oshblog(self):
    ...
    def test__convert_blog_front_page_text_oshblog(self):
    ...
    def test_rearrange_blog_director_corner_missing(self):
        """Test if we correctly handle a missing director's corner section"""
    ...
    def test_rearrange_blog_blog_entries_moved(self):
        """Test if the blog entries have been moved to the parent folder."""
        from osha.policy.upgrades import rearrange_blog

        rearrange_blog(self.portal)
        self.assertEquals(len(self.director_corner_en['blog'].keys()), 0)
        self.assertIn('blog-entry-1', self.director_corner_en.keys())
        self.assertIn('blog-entry-2', self.director_corner_en.keys())
    ...
    def test_rearrange_blog_hierarchy(self):
        ...
        # XXX: currently we're not deleting the canonical blog subfolder,
        # see rearrange_blog method for details
        self.assertEquals(
            sorted(self.director_corner_en.keys()),
            ['blog', 'blog-entry-1', 'blog-entry-2', 'front-page']
        )
        self.assertEquals(
    ...
    def test_rearrange_blog_translations_linked(self):
        ...
        translations = self.director_corner_en['front-page'].getTranslations()
        self.assertEquals(sorted(translations.keys()), ['de', 'en'])
    ...
    def test_rearrange_blog_front_page_content(self):
        ...
        self.assertEquals(
            self.director_corner_en['front-page'].Description(),
            "The aim of this blog is to bring you news about developments in occupational safety and health across the EU and beyond, and also about EU-OSHA initiatives and activities to fulfill our mission. We are one of the smallest EU agencies and cannot promise to reply to every comment, but we will read them and bear them in mind to shape our future work."
        )
        self.assertEquals(
        ...
        self.assertEquals(
            self.director_corner_de['front-page'].Description(),
            "The aim of this blog is to bring you news about developments in occupational safety and health across the EU and beyond, and also about EU-OSHA initiatives and activities to fulfill our mission. We are one of the smallest EU agencies and cannot promise to reply to every comment, but we will read them and bear them in mind to shape our future work."
        )
        self.assertEquals(
