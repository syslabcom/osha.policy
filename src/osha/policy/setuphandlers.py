...
def repositionActions(portal):
    ...
    # if xliff is installed, move site action xliff import into the
    # object actions dropdown
    ...
    # compare site actions in the header with footer actions and
    # remove duplications
...
def addExtraIndexes(self):
    ...
    if idx_id not in available:
        ...
        cat.manage_addProduct['PluginIndexes'].manage_addKeywordIndex(
            id=idx_id, extra=dict(indexed_attrs=idx_id))
    ...
    if 'getRemoteLanguage' not in available:
        ...
        cat.manage_addProduct['PluginIndexes'].manage_addKeywordIndex(
            ...
            extra={'indexed_attrs': 'getRemoteLanguage'})
...
def addProxyIndexes(self):
    ...
    logger.info(
        "NOT adding Proxy Indexes - using Keyword / Field Indexes instead")
    ...
    for data in field_index_data:
        ...
        cat.manage_addProduct['PluginIndexes'].manage_addFieldIndex(
            id=data['idx_id'])


def configureCountryTool(portal):
    """ Adds the relevant countries to the countrytool """
    ct = getToolByName(portal, 'portal_countryutils')
...
def configureSEOOptimizer(portal):
...
def configureCacheFu(portal):
    ...
    portal_cache_settings = getToolByName(
        portal, 'portal_cache_settings', None)
    ...
    downloads.setHeaderSetIdExpression(
        "python:object.portal_cache_settings.canAnonymousView(object) and 'cache-in-browser-1-hour' or 'no-cache'")
...
def modifySEOActionPermissions(portal):
